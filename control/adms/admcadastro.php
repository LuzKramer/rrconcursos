...
if (isset($_POST['cadastrar'])) {
    ...
    $check_sql = "SELECT * FROM tb_admlogin WHERE email = '$email'";
    $check_query = $mysqli->query($check_sql) or die("SQL code error: " . $mysqli->error);

    $existing_user = $check_query->num_rows;

    if ($existing_user > 0) {
        echo "<script>alert('Erro ao cadastrar! O email já está registrado.')</script>";
    } else {

        if ($mysqli->query("INSERT INTO tb_admlogin (nome, email, senha) VALUES ('$name', '$email', '$senha')")) {
            echo "<script>alert('cadastro sucedido.')</script>";
