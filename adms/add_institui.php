...
// Function to add a new "banca" to the database
function addinstituicao($conexao, $institui)
{
    $stmt = $conexao->prepare("INSERT INTO instituicao (nome_instituicao) VALUES (?)");
    $stmt->bind_param("s", $institui);
...
<body>
    ...
    <div class="main">
        <form method="POST">
            <h1>Adicione uma instituição</h1>
            <input type="text" size="255" name="institui" required>
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (addinstituicao($conexao, $institui)) {
        echo "<script>alert('instituição adicionada com sucesso!');</script>";
    } else {
        echo "<script>alert('Erro ao adicionar instituição.');</script>";
    }
}
$conexao->close();
?>