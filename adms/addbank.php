...
function addBanca($conexao, $banca)
{
    $stmt = $conexao->prepare("INSERT INTO bancas (nome_banca) VALUES (?)");
    $stmt->bind_param("s", $banca);
...
<body>
    ...
    <div class="main">
        <form method="POST">
            <h1>Adicione uma banca</h1>
            <input type="text" size="255" name="banca" required>
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (addBanca($conexao, $banca)) {
        echo "<script>alert('Banca adicionada com sucesso!');</script>";
    } else {
        echo "<script>alert('Erro ao adicionar banca.');</script>";
    }
}
$conexao->close();
?>